...
const ScrollbarContent = ({ bottomFooterRef, topBoxRef }: Props) => {
  ...
  return (
    <div className="flex flex-col px-6 py-8 gap-y-6 secondary-text overflow-y-scroll h-full">
        ...
        {personalInfoData[t("locale") as Locales].map((item, index) => (
          <div key={index}>
            <div className="flex items-center justify-between text-sm">
              <span className="font-medium primary-text">{item.label}</span>
          ...
          <div key={index * 7}>
            <div className="flex items-center text-sm gap-x-4">
              <span className="font-medium">
                <ThemeIcon>{<item.icon className="text-lg" />}</ThemeIcon>
              </span>
              <span>{item.value}</span>
            </div>
          </div>
      ...
      <div
        style={{
          marginBottom: bottomFooterRef.current ? bottomFooterRef.current.clientHeight : 100,
        }}
      >
        <a href="/en-resume.pdf" className="flex items-center gap-x-1 hover-primary-text">
          <span className="text-sm uppercase font-medium">{t("resume")}</span>
